...
library(corrplot)    ## to make correlation matrix
library(class)
...
corrplot(cor(subset(data,select = -c(Class))),method = "shade",main = "Correaltion Matrix")      ## all the variables are almost totally uncorrelated
...
## histogram for the target column of the original training data
dev.new(width = 1 ,height =5)
barplot(c(length(which(train$Class==0)),length(which(train$Class==1))),names.arg = c("0","1"),col = c("red","blue"),main = "Under Sampled Data",xlab = "Class (0:Non-Fraudulent, 1:Fraudulent)",ylab = "count")
...
## histogram for the target column of under sampled training data
dev.new(width = 1 ,height =5)
barplot(c(length(which(under$Class==0)),length(which(under$Class==1))),names.arg = c("0","1"),col = c("red","blue"),main = "Under Sampled Data",xlab = "Class (0:Non-Fraudulent, 1:Fraudulent)",ylab = "count")
...
## Using K-Nearest Neighbors machine learning algorithm to do classification
train_norm = as.data.frame(scale(train[,c(1:29)]))
test_norm = as.data.frame(scale(test[,c(1:29)]))
test_y = as.factor(test$Class)
train_y = as.factor(train$Class)
prediction = knn(train=train_norm, test=test_norm, cl=train_y,k=20)

confusion = table(prediction = prediction, actual = test_y)
confusion

precision = posPredValue(prediction, test$Class, positive="1")
recall = sensitivity(prediction, test$Class, positive="1")
F1= (2*precision*recall)/(precision+recall)
sprintf("F1 score for Decision Tree Algorithm is %f",F1)    ## F1-score for KNN algorithm is 82.9%
...
xgb = train(x = as.matrix(under1), y = under$Class,  tuneGrid = grid_tune,method= "xgbTree")
